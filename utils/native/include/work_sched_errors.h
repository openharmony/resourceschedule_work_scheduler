...
const inline std::map<int32_t, std::string> saErrCodeMsgMap = {
    ...
    {E_CHECK_WORKINFO_FAILED, "Check on workInfo failed. Current bundleUid and input uid do not match."},
    {E_ADD_REPEAT_WORK_ERR, "Calling startWork failed. The work has been already added."},
...
const inline std::map<int32_t, std::string> paramErrCodeMsgMap = {
    ...
    {E_IS_DEEP_IDLE_ERR, "The type of isDeepIdle must be boolean."},
