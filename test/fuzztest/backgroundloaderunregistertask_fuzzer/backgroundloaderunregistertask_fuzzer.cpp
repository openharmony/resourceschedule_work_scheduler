...
#include "backgroundloaderunregistertask_fuzzer.h"
...
namespace WorkScheduler {
    ...
    bool DoSomethingInterestingWithMyAPI(const uint8_t* data, size_t size)
    {
        ...
        uint32_t code = static_cast<int32_t>(IWorkSchedServiceIpcCode::COMMAND_UNREGISTER_TASK);

        BackgroundLoaderTaskInfo taskInfo(1, "TestAbility");
        if (!dataMessageParcel.WriteParcelable(&taskInfo)) {
            return false;
        }
