...
#include "backgroundloadergettaskinfo_fuzzer.h"
...
namespace WorkScheduler {
    ...
    bool DoSomethingInterestingWithMyAPI(const uint8_t* data, size_t size)
    {
        ...
        uint32_t code = static_cast<int32_t>(IWorkSchedServiceIpcCode::COMMAND_GET_TASK_INFO);

        int32_t taskId = 1;
        if (!dataMessageParcel.WriteInt32(taskId)) {
            return false;
        }
